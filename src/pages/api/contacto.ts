  ...
  try {
    ...
    if (botToken && chatId) {
      const text = [
        ...
        origen ? `Nos encontró vía: ${origen}` : '',
        '',
        `Mensaje: ${mensaje}`,
      ].filter(Boolean).join('\n');
...
<div style="max-width:500px;margin:30px auto;background:#fff;border-radius:12px;overflow:hidden;box-shadow:0 2px 12px rgba(0,0,0,0.08);">
  ...
  <div style="padding:24px;line-height:1.7;color:#333;font-size:14px;">
    ...
    ${telefono ? `<p><strong>Teléfono:</strong> <a href="tel:${encodeURIComponent(telefono)}" style="color:#4a6548;">${escapeHtml(telefono)}</a></p>` : ''}
    ${email ? `<p><strong>Email:</strong> <a href="mailto:${encodeURIComponent(email)}" style="color:#4a6548;">${escapeHtml(email)}</a></p>` : ''}
    ${servicio ? `<p><strong>Servicio de interés:</strong> ${escapeHtml(servicio)}</p>` : ''}
    ${origen ? `<p><strong>Nos encontró vía:</strong> ${escapeHtml(origen)}</p>` : ''}
